...
export class LoginService { 
  ...
  login(credentials: { numemp: string; contrasena: string }): Observable<any> {
    ...
    return this.http.post(`${this.apiUrl}/login`, credentials).pipe(
      tap((response: any) => {
        ...
        this.saveUserData(response.token, response.usuario.userLevel, response.usuario.nombre, credentials.numemp.toUpperCase());
      }),
      catchError((error) => {
  ...
  saveUserData(token: string, userLevel: number, username: string, numemp: string): void {
